...
class Trainer(object):
    ...
    def save(self, itrs=None):
        # Save model state without the DataParallel wrapper
        model_state = self.model.module.state_dict() if isinstance(self.model, nn.DataParallel) else self.model.state_dict()
        ema_model_state = self.ema_model.module.state_dict() if isinstance(self.ema_model, nn.DataParallel) else self.ema_model.state_dict()
        data = {
            'step': self.step,
            'model': model_state,
            'ema': ema_model_state
    ...
    def train(self):
        ...
        pbar = tqdm(range(self.step, self.train_num_steps + 1), desc='LOSS') #self.train_num_steps+1
    ...
    def test(self, t, num_samples=1):
        ...
        # If using DataParallel, make sure training mode is set correctly
        if isinstance(self.ema_model, nn.DataParallel):
            self.ema_model.module.training = False
        else:
            self.ema_model.training = False
