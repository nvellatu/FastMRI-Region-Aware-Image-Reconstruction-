...
def recon_kspace_cold_diffusion(
    ...
    # If there are multiple GPUs, use DataParallel to utilize all of them
    if torch.cuda.device_count() > 1:
        print(f"Using {torch.cuda.device_count()} GPUs!")
        net = nn.DataParallel(net)  # Wrap the model with DataParallel
    ...
    with torch.no_grad():
        for idx, data in enumerate(dataloader):
            ...
            # network forward
            xt, direct_recons, sample_imgs = net.module.sample(kspace, mask, mask_fold, t=timesteps)
